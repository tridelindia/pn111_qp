  ...
  ngOnInit(): void {
    ...
    this.stationConfig.getStationNames().subscribe((stationConfig) => {
      ...
      this.reportService.getAllSensorData().subscribe((data) => {
        ...
        console.log('Filtered Live Buoys: ', this.livefetchedBuoys);
        ...
        if (this.livefetchedBuoys.length !== 0) {
          ...
          for (let index = 0; index < this.livefetchedBuoys.length; index++) {
            const liveLocation = this.livelocationbuoys[index];
            ...
            const stationName = (this as any)[`stationName${index + 1}`];
 
            console.log(`Checking range for buoy ${index + 1}:`, {
              liveLocation,
              center,
              wrange,
              drange,
            ...
            const buoyRange = this.mapService.checkBuoyRange(
              liveLocation, // ✅ Use already assigned variable
              center,
              drange,
              wrange,
              stationName
            ...
            console.log(`Range status for buoy ${index + 1}:`, buoyRange);
            (this as any)[`buoy${index + 1}range`] = buoyRange;
            this.buoyRanges.push(buoyRange);
 
            this.fetchedBuoys.forEach((configBuoy) => {
              ...
              if (liveMatch) {
                ...
                if (driftDistance <= wrange) {
                  ...
                  driftStatus = `crossed out of\nwarning range`;
                  driftClass = 'text-warning';
                } else if (driftDistance > drange) {
                  driftStatus = `crossed out of\ndanger range`;
                  driftClass = 'text-danger';
                }
 
                const driftText = `${driftDistance.toFixed(
                  1
                )} m ${driftDirection}\n(${driftStatus})`;
                ...
                console.log(
                  `Drift for ${configBuoy.station_name}: ${driftText}`
                );
              }
            });
          }
          ...
          console.log('Final Buoy Data: ', this.buoys);
        }
      });
      ...
      //   const rangeStatus =
      //     this.buoyRanges.find((r) =>
      //       r.includes(configBuoy.station_name)
      //     ) || '';
 
      //   let driftText = '';
 
      //   if (rangeStatus.includes('Danger Range')) {
      //     // Show only drift distance if buoy is in danger
      //     driftText =
      //       this.buoyDrifts.find((d) => d.name === configBuoy.station_name)
      //         ?.drift || 'N/A';
      //   } else {
      //     // Otherwise, show the range message
      //     driftText = rangeStatus;
      //   }
 
      //   return {
      //     name: configBuoy.station_name,
      //     lat: liveMatch?.lat || configBuoy.lat_dd,
      //     long: liveMatch?.lon || configBuoy.lon_dd,
      //     status: statusText,
      //     drift: driftText,
      //     image:
