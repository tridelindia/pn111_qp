...
export class NotificationComponent implements OnInit {
  ...
  onStationChange(event: any) {
    if (event.value && event.value.length > 0) {
      const selectedStations = this.stations.filter(station => event.value.includes(station.id));
      this.newNotification.station_id = event.value;
      this.newNotification.station_name = selectedStations.map(station => station.name);
    } else {
