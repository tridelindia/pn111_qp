...
export class AnalyticsComponent implements OnInit {
  ...
  changetiMultiStationView(val:string) {
    this.isLoad = true;
    if (this.selectedMultiStationParam.includes('current')) {
      if(val==='init'){
        this.selectedBin = this.Bins[0].name;
      }
      this.isMultiCurrent = true;

      this.changewithBinCompare('')
    } else {
    ...
    forkJoin(requests).subscribe(
      (responses: any[]) => {
        ...
        console.log('Final Structured Data:', result, this.selectedMultiStationParam);
      },
      error => {
  ...
  isViewChange(value: string) {
    ...
    setTimeout(() => {
      console.log("sss", this.stations, this.singleStation, this.selectedStation);
      ...
      if(value ==='single'){
        ...
        const stat = this.stations.filter(item=> item.name=== this.selectedStation[0]);
      const ssdgstat = this.ssstations.filter(item => item.station_name === this.selectedStation[0])

        this.isMeteriology = ssdgstat[0].sensors.includes('meteorology');
      }
    }, 100);
  }
  fromDate!: string;
...
onPlotSelect(event: DropdownChangeEvent): void {
  ...
  changewithBinCompare(val:string) {
...
if(val==='html'){
  ...
  changetiMulOne() {
    ...
    this.selectedBin_multiy1 = this.Bins[0].name;
    this.selectedBin_multiy2 = this.Bins[0].name;
...
getFilteredScatterYParams(): any[] {
  ...
  onScatterChange() {
    ...
    this.selectedBinScatterX = this.Bins[0].name;
    this.selectedBinScatterY = this.Bins[0].name;
    console.log(this.selectedBinScatterX, this.selectedBinScatterY);
